...
| 1 — Domain foundation | Implemented slice | Asset types/registry/recursive explicit scanner, SHA256 identity, compute profile, capabilities, Recipe v2, bridge handlers, focused tests |
| 2 — Dual-mode UI + local library | Implemented slice | Persisted Discover/Library surface; Discover exposes Models/LoRAs/Recipes/Workflows directly while Library exposes Models/LoRAs/Styles/Generate/Automate; existing local galleries and LoRA stack remain the execution source |
| 3 — Provider/download foundation | Implemented slice, hardened | Backend Civitai + Hugging Face providers/tokens, isolated cached search, persistent atomic queue, real resumable pause/cancel, safe paths, pre-install SHA256 verification, registration, focused tests |
| 4 — Models & LoRAs discovery UX | Implemented slice | Provider-neutral cards, file variants, architecture gate, compute-aware recommendation, immediate Library refresh, SHA256-backed installed state, authenticated browsing, and Civitai cursor/Hugging Face page pagination with Load More |
| 5 — Recipe & Prompt Discovery | Complete | Portable Recipe v2 Save/Load/Recreate, image-metadata import, Civitai `withMeta` image browsing with cursor pagination and visible generation settings, exact Civitai model-version resolution, missing-file dialog with real source links, Download Manager installs, local model/LoRA replacement, optional LoRA skipping, optional Lexica search, managed Recipe Library saves, and installed badges |
...
| 7 — Official workflow discovery | Complete | Auto-loaded official ComfyUI picker catalog (583 entries at verification), category/tag/open-source/API filters, thumbnail/model metadata, SSRF/redirect checks, content-addressed JSON download, dependency visibility, and fixed-root atomic JSON/PNG Library import; imported files never auto-execute or install nodes |
| 8 — Workflow compatibility | Complete (fail-closed scope) | API/UI/PNG metadata intake, deterministic IR/compiler, connected-graph validation, dependency extraction, security checks, and exactly `NATIVE`, `ADAPTABLE`, `COMFY_ONLY`, or `INVALID`; advanced/disconnected/unsupported subgraphs remain non-executable |
| 9 — Native workflow execution | Complete (narrow basic text-to-image scope) | Lossless native graphs compile to Recipe v2, show their model/prompt/sampler/scheduler/size before use, then open the visible Generate settings for user review; execution remains on the existing ComfyUI-backed path |
| 10 — Automation | Complete, bounded | Recipe v2 batch/folder/matrix automation, deterministic seed sweeps, bounded previews/queues, real cancellation, and export reuse the existing ComfyUI-backed worker |

Review verification boundary: 64 focused discovery/backend tests pass; the desktop TypeScript/Vite production build, Rust `cargo check`, Rust formatting check, and `git diff --check` pass. The full backend suite reaches 1,086 passed / 3 skipped with four pre-existing HiDream O1 resolution expectation failures outside Discover/Library. The native Tauri UI was visually checked at 1440×900 for Models, LoRAs, Recipes, and Workflows; this caught and fixed clipped Discover tabs and shrinking Recipe cards. Manual assistive-technology testing was not performed. Unsupported workflow semantics remain visibly blocked rather than partially executed.
