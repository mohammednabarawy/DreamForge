...
```bash
./dreamforge.sh
# or double-click DreamForge.command
```

If `@tauri-apps/cli` reports a missing native binding, reinstall desktop deps with **arm64** Node:
...
If port **5173** is stuck after a crash:
