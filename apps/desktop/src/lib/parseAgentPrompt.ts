...
function applyEditDefaults(
  ...
  Object.assign(patch, {
    edit_type: patch.edit_type ?? "auto",
    style: patch.style ?? "image_edit",
  });
}
...
function applyIdentityDefaults(
  ...
  const editType = patch.edit_type ?? settings.edit_type;
  if (editType === "inpaint") return;
  ...
  if (inputImage) {
    ...
    if (kontext) {
      ...
      patch.edit_strength = patch.edit_strength ?? 1.0;
      patch.cn_selection = "None";
      patch.cn_type = "None";
      patch.steps = patch.steps ?? 20;
