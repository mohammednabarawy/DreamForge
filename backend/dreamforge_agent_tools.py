...
def apply_recipe_defaults(args, *, text_pipeline: bool = False):
    ...
    if hasattr(args, "performance") and getattr(args, "performance", "Lightning") == "Lightning":
    ...
    if not text_pipeline and hasattr(args, "aspect_ratio") and getattr(args, "aspect_ratio", "768x768") == "768x768":
...
def list_loras_for_agent(*, limit: int | None = None) -> list[dict[str, Any]]:
    """Installed LoRA files under models/loras."""
...
def build_agent_catalog(*, style_limit: int = 40, lora_limit: int = 80) -> dict[str, Any]:
    ...
    return {
        ...
        "workflow_modes": {
            ...
            "upscale": "upscale_image with upscale_method ultimate_sd_upscale.",
