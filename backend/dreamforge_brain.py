...
class BrainProvider:
    ...
    def think(self, prompt: str, system_prompt: str, *, max_tokens: int | None = None) -> str:
...
class EmbeddedLlamaCppProvider(BrainProvider):
    ...
    def _get_model_path(self) -> Path:
        ...
        for root_name in ("llm", "LLM"):
            for base_dir in (Path(MODELS_ROOT),):
    ...
    def think(self, prompt: str, system_prompt: str, *, max_tokens: int | None = None) -> str:
        ...
        try:
            ...
            _EMBEDDED_LLM_SERVER.handle_chat_completions(payload, callback)
        except Exception as e:
...
def heuristic_brain_decision(
    ...
    if "upscale" in operations:
        patch.setdefault("upscale_method", "ultimate_sd_upscale")
    ...
    if "outpaint" in operations:
        ...
        existing_edit = str(settings.get("edit_type") or "").lower()
        model_hint = " ".join(
            str(settings.get(key) or "") for key in ("model", "base_model", "engine_name")
        ).lower()
        if existing_edit == "qwen_edit" or ("qwen" in model_hint and "edit" in model_hint):
            patch.setdefault("edit_type", "qwen_edit")
            patch.setdefault("cn_type", "qwen_edit")
            patch.setdefault("cn_selection", "Custom...")
            patch.setdefault("performance", "Lightning")
        else:
            patch.setdefault("edit_type", "auto")

    blueprint_settings = dict(settings)
    for key, value in patch.items():
        if value is not None:
            blueprint_settings[key] = value
    try:
        ...
        blueprint = build_live_workflow_blueprint(
            ...
            current_settings=blueprint_settings,
        )
...
class AiBrain:
    ...
    def think(self, prompt: str, system_prompt: str, *, max_tokens: int | None = None) -> str:
        ...
        try:
            return self.provider_inst.think(prompt, system_prompt, max_tokens=max_tokens)
        except Exception as e:
            ...
            if not isinstance(self.provider_inst, EmbeddedLlamaCppProvider) and XLC_AVAILABLE:
                try:
                    ...
                    return fallback.think(prompt, system_prompt, max_tokens=max_tokens)
