...
def build_parser():
    ...
    parser.add_argument("--performance", default="Lightning", help="Lightning, Speed, Quality, or Custom...")
    ...
    parser.add_argument(
        "--reference-image",
        default=None,
        help="Primary reference photo for IP-Adapter / identity-guided generate (face preservation)",
    )
    parser.add_argument(
        "--reference-images",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "Auto-tune for VRAM target: 16gb/8gb/5gb (NVIDIA), "
            "mps_* (Apple Silicon unified memory tiers), or mps/auto detect"
        ),
    )
    parser.add_argument(
        "--stream-file",
...
def _normalize_vram_profile(profile):
    from dreamforge_vram_profiles import normalize_vram_profile

    return normalize_vram_profile(profile)
...
def _resolve_model(model_name, profile, job_dict):
    ...
    speed_pref = normalize_routing_speed_preference(job_dict.get("performance"))
    try:
...
def _apply_edit_recipe_settings(settings: dict, model: dict, job) -> dict:
    ...
    if explicit_sampling and custom_perf:
        return settings
    try:
    ...
    recipe_family = "qwen_image_edit" if family.lower().startswith("qwen") else family
    recipe_type = "qwen_edit" if family.lower().startswith("qwen") else edit_type
    recipe = edit_recipe(recipe_family, recipe_type)
    if not recipe:
        return settings
    out = dict(settings)
    ...
    if recipe_family == "qwen_image_edit":
        from dreamforge_prompt.pipeline import _qwen_edit_needs_quality_pass
        prompt_text = str(getattr(job, "prompt", "") or "")
        global_edit = _qwen_edit_needs_quality_pass(prompt_text)

        perf = str(getattr(job, "performance", "") or "").strip().lower()
        out["qwen_image_shift"] = float(recipe.get("qwen_image_shift", out.get("qwen_image_shift", 3.1)))
        if global_edit:
...
def build_plan(base_args, data=None):
    ...
    route_actions = model_fallback_actions(
        ...
        capabilities,
        getattr(job, "vram_profile", "auto"),
        normalize_routing_speed_preference(getattr(job, "performance", None)),
    )
    ...
    try:
        ...
        if str(model.get("family") or "").lower() == "qwen_image_edit":
            current_settings.setdefault("edit_type", "qwen_edit")
            current_settings.setdefault("cn_type", "qwen_edit")
        ...
        from dreamforge_workflow_planner import filter_operations_for_plan_mode

        operations = filter_operations_for_plan_mode(
            operations,
    ...
    return {
        ...
        "status": "planned",
        "mode": mode,
        "escalation_reason": settings.get("escalation_reason"),
        "prompt": prompt,
