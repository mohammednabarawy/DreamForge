...
from _paths import BACKEND_ROOT, MODELS_ROOT, PROJECT_ROOT, extend_sys_path

extend_sys_path()

MODEL_EXTENSIONS = {".safetensors", ".ckpt", ".pt", ".pth", ".bin", ".gguf", ".svdq", ".nunchaku"}
...
def detect_vram_profile():
    from dreamforge_vram_profiles import detect_vram_profile as _detect

    return _detect()


def normalize_vram_profile(profile):
    from dreamforge_vram_profiles import normalize_vram_profile as _normalize
...
def _profile_vram_budget_gb(profile):
    from dreamforge_vram_profiles import profile_vram_budget_gb

    return profile_vram_budget_gb(profile)
...
MODEL_DEPENDENCIES = {
    ...
    "flux_fill": [
        ...
        },
        {
            ...
            "note": "Flux T5-XXL scaled fp8 text encoder (VRAM-friendly).",
        },
    ],
    "flux": [
        {
    ...
    "ideogram4": [
        {
            "id": "unet_ideogram4_unconditional",
            "relative": "diffusion_models/ideogram4_unconditional_fp8_scaled.safetensors",
            "note": "Required unconditional UNet for Ideogram 4 dual-model CFG.",
        },
        {
            "id": "clip_qwen3vl_ideogram4",
            "relative": "text_encoders/qwen3vl_8b_fp8_scaled.safetensors",
            "note": "Ideogram 4 Qwen3-VL text encoder (CLIP type ideogram4).",
        },
        {
            "id": "vae_flux2",
            "relative": "vae/flux2-vae.safetensors",
            "note": "Flux 2 VAE used by Ideogram 4 decode.",
        },
    ],
    "z_image": [
        {
            "id": "clip_z_image_qwen3_4b_fp4",
            "relative": "text_encoders/qwen_3_4b_fp4_mixed.safetensors",
            "note": "Required Qwen 3 4B FP4 mixed text encoder for Z-Image Turbo.",
        },
        {
            "id": "vae_flux_ae",
            "relative": "vae/ae.safetensors",
...
QWEN_EDIT_LIGHTNING_LORA = {
    ...
    "note": "Qwen Image Edit 2511 Lightning LoRA (4-step Speed preset).",
}

QWEN_EDIT_LIGHTNING_LORA_8STEP = {
...
def qwen_lightning_lora_requirement(performance: str | None) -> dict:
    """Prefer 8-step Lightning for the default quality-biased Lightning preset."""
    perf = str(performance or "").strip().lower()
    if perf == "lightning":
...
def normalize_routing_speed_preference(speed_preference: str | None) -> str:
    """Default to Lightning/fast routing unless the user chose a concrete tier."""
    perf = str(speed_preference or "").strip()
    if not perf or perf.lower() in {"custom...", "custom"}:
        return "Lightning"
    lowered = perf.lower()
    if lowered in {"lightning", "lcm"}:
        return "Lightning"
    if lowered in {"speed", "extreme speed"}:
        return "Speed"
    return perf
...
def pick_best_qwen_edit_model(gallery: list) -> str:
    ...
    for item in gallery:
        if not isinstance(item, dict):
            continue
        score = score_qwen_edit_gallery_item(item)
        if score > best_score:
            best_score = score
            best_name = str(item.get("engine_name") or item.get("relative_path") or "")
    return best_name


def qwen_fused_lightning_model(model_name: str | None) -> bool:
    """True for Qwen Edit checkpoints that already include the Lightning distillation."""
    name = str(model_name or "").strip().lower()
    if "qwen" not in name or "edit" not in name or "lightning" not in name:
        return False
    return any(token in name for token in ("fp8", "e4m3fn", "scaled", "fused"))
...
def qwen_lightning_lora_present(*, min_bytes: int = 800 * 1024 * 1024) -> bool:
    ...
    try:
        ...
        for path in lora_dir.iterdir():
            if path.suffix.lower() not in (".safetensors", ".gguf", ".svdq", ".nunchaku"):
                continue
...
def qwen_specific_lightning_lora_present(req: dict, *, min_bytes: int = 800 * 1024 * 1024) -> bool:
    """True if the requested 4-step/8-step Lightning LoRA or a known alias is present."""
    if companion_file_present(req, min_bytes=min_bytes):
        return True
    for alt in COMPANION_ALTERNATE_PATHS.get(req.get("id", ""), []):
        if companion_file_present({**req, "relative": alt}, min_bytes=min_bytes):
            return True
    return False
...
COMPANION_ALTERNATE_PATHS: dict[str, list[str]] = {
    ...
    "lora_qwen_edit_lightning_4step": [
        ...
        "loras/Qwen-Image-Edit-2509-Lightning-4steps-V1.0-fp32.safetensors",
    ...
    "lora_qwen_edit_lightning_8step": [
        ...
        "loras/Qwen-Image-Edit-2511-Lightning-8steps-V1.0-fp32.safetensors",
...
def check_model_dependencies(model, *, performance: str | None = None):
    ...
    if (
        ...
        and qwen_lightning_lora_requested(performance)
    ...
    for req in reqs:
        if req.get("optional"):
            continue
        if req.get("id") in {
            QWEN_EDIT_LIGHTNING_LORA["id"],
            QWEN_EDIT_LIGHTNING_LORA_8STEP["id"],
        }:
            if qwen_specific_lightning_lora_present(req):
                continue
...
def route_best_model(capabilities, vram_profile="16gb", speed_preference="Quality", exclude_families=None):
    ...
    for category in GENERATION_MODEL_CATEGORIES:
        for item in inventory["categories"].get(category, []):
            ...
            name = (model.get("name") or "").lower()
            if "qwen-image-edit-2511" in name or "qwen_image_edit_2511" in name:
                model["profile_score"] += 25
            if qwen_fused_lightning_model(name):
                model["profile_score"] += 18
            
            if speed_preference == "Lightning":
                if name.endswith(".gguf") or "q4_k_m" in name or "q4_k_s" in name:
                    model["profile_score"] += 45
                if any(token in name for token in ("schnell", "turbo", "lightning", "dmd2", "hyper")):
                    model["profile_score"] += 28
                model["profile_score"] += min(25, max(0, (9000 - (model.get("size_mb") or 0))) / 350)
                if qwen_fused_lightning_model(name):
                    model["profile_score"] -= 12
                if "fp8" in name and (model.get("size_mb") or 0) > 14000:
                    model["profile_score"] -= 35
            elif speed_preference == "Speed":
