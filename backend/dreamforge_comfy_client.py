...
COMFY_EXECUTION_TIMEOUT_S = float(os.environ.get("DREAMFORGE_COMFY_EXECUTION_TIMEOUT_S", "1800"))
...
class ComfyHungError(TimeoutError):
    """Raised when a Comfy prompt stops making progress for too long."""


def is_comfy_oom_error(exc: BaseException | str | None) -> bool:
...
def _ensure_managed_comfy_alive(*, prompt_id: str = "") -> None:
    """Raise when the managed Comfy subprocess died mid-prompt."""
    try:
        ...
        tail = comfy_server_log_tail(max_chars=4000)
        message = "ComfyUI process exited during generation."
        if is_comfy_oom_error(tail):
            message = "ComfyUI ran out of GPU memory and stopped."
        raise ComfyExecutionError(
            message,
            prompt_id=str(prompt_id),
            details={"comfy_log_tail": tail[-2000:] if tail else ""},
        )
...
class ComfyClient:
    ...
    def queue(self, *, timeout_s: float = 10.0) -> dict[str, Any]:
        return _http_json("GET", f"{self.base_url}/queue", None, timeout_s=timeout_s)

    def interrupt_if_stalled(self, *, stall_s: float = COMFY_HUNG_STALL_S) -> bool:
        """Best-effort interrupt when the queue still has running work."""
        try:
            running = self.queue().get("queue_running") or []
            if running:
                self.interrupt()
                return True
        except Exception as exc:
            _log.warning("Failed to interrupt stalled Comfy prompt: %s", exc)
        return False
    ...
    def wait_for_outputs(
        ...
        *,
        ...
        stall_s: float = COMFY_HUNG_STALL_S,
    ) -> dict[str, Any]:
        ...
        while time.time() < deadline:
            _ensure_managed_comfy_alive(prompt_id=str(prompt_id))
            try:
            ...
            snapshot = json.dumps(node.get("status") if isinstance(node, dict) else None, default=str)
            if snapshot != last_snapshot:
                last_snapshot = snapshot
                last_change = time.time()
            elif time.time() - last_change >= float(stall_s):
                self.interrupt_if_stalled(stall_s=stall_s)
                raise ComfyHungError(
                    f"ComfyUI prompt {prompt_id} stalled for {int(stall_s)}s without progress"
                )
        ...
        self.interrupt_if_stalled(stall_s=stall_s)
        raise ComfyHungError(f"Timed out waiting for Comfy outputs ({prompt_id}). Last={last}")
        ...
        if on_event is not None:
            try:
                ...
                if submitted is not None:
                    _log.warning(
                        "Comfy live preview WebSocket ended early (prompt %s); "
                        "falling back to HTTP history polling: %s",
                        submitted.prompt_id,
                        exc,
                    )
                    if on_event is not None:
                        ...
                        if len(detail) > 120:
                            detail = detail[:117] + "…"
                        message = (
                            "Live preview unavailable; still generating via Comfy (HTTP)…"
                        )
                        if detail:
