...
_OBJECT_INFO_CACHE: dict[str, object] = {"fetched_at": 0.0, "payload": None}
_OBJECT_INFO_TTL_SEC = 90.0
...
class ManagedComfyServer:
    ...
    def start(self, timeout_s: float = 30.0) -> None:
        with _managed_comfy_start_lock:
            ...
            stopped: list[int] = []
            stopped.extend(cleanup_all_foreign_comfy_servers(force=True, exclude_pids=exclude))
            ...
            if stopped:
                unique = list(dict.fromkeys(stopped))
                print(
                    f"[DreamForge] Stopped {len(unique)} existing local ComfyUI instance(s) "
                    f"before starting managed server (PIDs: {', '.join(map(str, unique))})",
                    file=sys.stderr,
                )
...
_managed_comfy_start_lock = threading.Lock()


def _comfy_launch_extra_args() -> tuple[str, ...]:
    from dreamforge_comfy_launch import comfy_launch_extra_args

    return comfy_launch_extra_args()


def get_default_comfy_server() -> ManagedComfyServer:
    global _DEFAULT_SERVER
    if _DEFAULT_SERVER is None:
        cfg = ComfyServerConfig(extra_args=_comfy_launch_extra_args())
...
def boot_managed_comfy_server(
    *,
    ...
    extra_args = _comfy_launch_extra_args()
    if extra_args:
        from dreamforge_comfy_launch import comfy_launch_summary
...
def probe_comfy_http_base_url(
    *,
    host: str = "127.0.0.1",
    port: int = 8188,
    timeout_s: float = 2.0,
) -> str | None:
    """Return Comfy base URL when an existing local server responds; never spawns one."""
...
def fetch_comfy_object_info(
    *,
    base_url: str | None = None,
    timeout_s: float = 20.0,
    use_cache: bool = True,
) -> dict:
    """Read /object_info from an already-running Comfy server (bridge-safe)."""
    from dreamforge_comfy_client import ComfyClient

    now = time.monotonic()
    cached = _OBJECT_INFO_CACHE.get("payload")
    if (
        use_cache
        and isinstance(cached, dict)
        and now - float(_OBJECT_INFO_CACHE.get("fetched_at") or 0.0) < _OBJECT_INFO_TTL_SEC
    ):
        return cached

    url = base_url or probe_comfy_http_base_url()
    if not url:
        raise RuntimeError(
            "ComfyUI is not responding on localhost. Start or restart the GPU engine first."
        )
    payload = ComfyClient(url).object_info(timeout_s=float(timeout_s))
    _OBJECT_INFO_CACHE["fetched_at"] = now
    _OBJECT_INFO_CACHE["payload"] = payload
    return payload


def invalidate_comfy_object_info_cache() -> None:
    _OBJECT_INFO_CACHE["fetched_at"] = 0.0
    _OBJECT_INFO_CACHE["payload"] = None


def comfy_server_log_tail(*, max_chars: int = 4000) -> str:
    """Return the tail of the managed Comfy server log (if present)."""
    global _DEFAULT_SERVER
    paths: list[Path] = []
    server = _DEFAULT_SERVER
    if server is not None and server.config.log_path is not None:
        paths.append(Path(server.config.log_path))
    paths.append(BACKEND_ROOT / "outputs" / "dreamforge" / "logs" / "comfy.server.log")
    seen: set[str] = set()
    for path in paths:
        key = str(path)
        if key in seen:
