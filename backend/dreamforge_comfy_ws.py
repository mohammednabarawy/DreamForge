...
class ComfyPromptStreamSession:
    ...
    def _check_stalled(self) -> None:
        if not self._prompt_id:
            return
        if time.time() - self._last_activity < float(COMFY_HUNG_STALL_S):
            return
        try:
            from dreamforge_comfy_client import ComfyClient

            ComfyClient(self.base_url).interrupt_if_stalled()
        except Exception:
            pass
        from dreamforge_comfy_client import ComfyHungError

        raise ComfyHungError(
            f"ComfyUI prompt {self._prompt_id} stalled for {int(COMFY_HUNG_STALL_S)}s without progress"
        )
    ...
    def wait_until_done(self, *, history_poll: HistoryPollFn | None = None) -> None:
        ...
        while time.time() < deadline:
            self._check_stalled()
            if self._prompt_id:
                from dreamforge_comfy_client import _ensure_managed_comfy_alive

                _ensure_managed_comfy_alive(prompt_id=str(self._prompt_id))
            if self._error is not None:
                raise RuntimeError(str(self._error)) from self._error
        ...
        raise ComfyHungError(f"Timed out waiting for Comfy prompt {self._prompt_id}")
