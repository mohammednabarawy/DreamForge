    ...
    *,
    ...
    for field in CONTRACT_FIELDS:
        if field in proposed and _meaningful(proposed.get(field)):
            if field == "model" and _same_model_name(
                _stringify(proposed.get(field)),
                _stringify(original.get(field)),
            ):
                if field in original:
                    preserved_fields.append(field)
            elif _stringify(proposed.get(field)) != _stringify(original.get(field)):
    ...
    if original_model and (not proposed_model or _same_model_name(proposed_model, original_model)):
...
def _same_model_name(left: str, right: str) -> bool:
    lhs = _model_basename(left).lower()
    rhs = _model_basename(right).lower()
