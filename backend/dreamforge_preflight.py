...
from _paths import BACKEND_ROOT, MODELS_ROOT, PROJECT_ROOT
...
def _estimate_required_vram_gb(model: Mapping[str, Any]) -> float | None:
    ...
    if family == "ideogram4" or "ideogram4" in low or family.startswith("ideogram"):
        # Conditional + unconditional UNets and Qwen3VL CLIP load together during sampling.
        return 14.5
...
def _torch_supports_nvfp4() -> bool:
    try:
        import torch  # local import; not required when called from CLI inventory
    except Exception:
        return False
    return bool(hasattr(torch, "float4_e2m1fn_x2"))
...
def run_preflight(
    ...
    * **VRAM headroom** - warning only; the GPU profile decides the final
    ...
    low_name = _model_basename(model).lower()
    if "nvfp4" in low_name and not _torch_supports_nvfp4():
        result.errors.append(
            unsupported_model_format(
                name,
                "NVFP4 models require a Torch build with torch.float4_e2m1fn_x2; "
                "the embedded local runtime does not provide that dtype.",
                job_id=job_id,
            )
        )
        result.errors[-1]["suggestions"] = [
            "Use the installed Qwen fp8 or GGUF edit model for this run.",
            "Update the embedded local Torch/Comfy runtime to a build that supports NVFP4 FP4 dtypes.",
            "After changing the runtime, restart the GPU engine so ComfyUI reloads Torch.",
        ]
