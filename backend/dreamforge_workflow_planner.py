...
TEMPLATE_REGISTRY: dict[str, WorkflowTemplateSpec] = {
    ...
    "qwen_image_edit": WorkflowTemplateSpec(
        ...
        operation="edit_image",
        mode="edit",
        summary="Text-aware Qwen Image Edit using native Qwen image-edit text encoding and optional Lightning LoRA.",
        builder="comfy_qwen_image_edit | comfy_qwen_image_edit_plus",
        node_pattern=[
        ...
        required_inputs=["input_image", "prompt"],
        optional_inputs=["reference_images", "edit_strength", "qwen_scale_megapixels"],
        required_models=["qwen_image_edit_unet", "qwen_text_encoder", "qwen_image_vae"],
        krita_alignment="Uses DreamForge/Krita-style Qwen edit recipe with AuraFlow sampling, CFGNorm, and local companion model checks.",
        research_basis=["krita_ai_diffusion", "ComfyUI native Qwen image-edit nodes", "workflow_research:qwen"],
    ),
    "inpaint_repair": WorkflowTemplateSpec(
        ...
        required_models=["flux_fill_checkpoint", "clip", "vae"],
    ...
    "z_image_txt2img": WorkflowTemplateSpec(
        id="z_image_txt2img",
        label="Z-Image Text to Image",
        operation="generate_image",
        mode="generate",
        summary="Z-Image txt2img pipeline using Qwen text encoder and AE VAE.",
        builder="comfy_z_image_txt2img",
        node_pattern=["UNETLoader", "CLIPLoader", "EmptySD3LatentImage", "KSampler", "VAEDecode", "SaveImage"],
        required_inputs=["prompt", "width", "height", "model"],
        required_models=["checkpoint_or_unet", "clip", "vae"],
        research_basis=["z-image official workflows"],
    ),
    "kandinsky5_txt2img": WorkflowTemplateSpec(
        id="kandinsky5_txt2img",
        label="Kandinsky 5 Text to Image",
        operation="generate_image",
        mode="generate",
        summary="Kandinsky 5 dual-CLIP txt2img pipeline.",
        builder="comfy_kandinsky5_txt2img",
        node_pattern=["UNETLoader", "DualCLIPLoader", "EmptyLatentImage", "KSampler", "VAEDecode", "SaveImage"],
        required_inputs=["prompt", "width", "height", "model"],
        required_models=["checkpoint_or_unet", "clip_l", "t5", "vae"],
        research_basis=["kandinsky5 official workflows"],
    ),
    "chroma_txt2img": WorkflowTemplateSpec(
        id="chroma_txt2img",
        label="Chroma Text to Image",
        operation="generate_image",
        mode="generate",
        summary="Chroma HD fast txt2img pipeline.",
        builder="comfy_flux_dev_txt2img",
        node_pattern=["UNETLoader", "DualCLIPLoader", "EmptySD3LatentImage", "KSampler", "VAEDecode", "SaveImage"],
        required_inputs=["prompt", "width", "height", "model"],
        required_models=["checkpoint_or_unet", "clip_l", "t5", "vae"],
...
# Style presets often include "highly detailed"; bare "detail" must not route to Impact Pack.
_FACE_DETAIL_PHRASES = (
    "face detail",
    "facial detail",
    "detail repair",
    "repair face",
...
_PRIMARY_INPAINT_OPS = frozenset({"inpaint", "remove_object", "outpaint"})
_TEXT_PLAN_WORKFLOW_MODES = frozenset(
    {"arabic_text_composite", "text_integrate", "arabic_poster", "exact_text", "poster"}
)
_COMPOSITE_PLAN_WORKFLOW_MODES = frozenset(
    {"area", "area_composition", "composite", "composition", "composite_layers"}
)


def filter_operations_for_plan_mode(
    operations: list[str],
    *,
    mode: str,
    has_image: bool,
    ...
    """Trim planned operations to what the active studio mode actually needs."""
    wf = str(workflow_mode or "").lower()
    ops = list(operations)

    if mode == "generate" and not has_image:
        if wf != "face_detail":
            ops = [op for op in ops if op != "face_detail"]
        return ops

    if mode == "inpaint":
        allowed = set(_PRIMARY_INPAINT_OPS)
        if wf in _TEXT_PLAN_WORKFLOW_MODES:
            allowed.add("text_integrate")
        if wf in _COMPOSITE_PLAN_WORKFLOW_MODES:
            allowed.add("composite_layers")
        if wf in {"style_transfer", "restyle", "kontext", "flux_kontext_edit"}:
            allowed.update({"style_transfer", "edit_image"})
        filtered = [op for op in ops if op in allowed]
        return filtered or ["inpaint"]

    if mode == "edit":
        if wf not in _TEXT_PLAN_WORKFLOW_MODES:
            ops = [op for op in ops if op != "text_integrate"]
        if wf not in _COMPOSITE_PLAN_WORKFLOW_MODES:
            ops = [op for op in ops if op != "composite_layers"]
        return ops or (["edit_image"] if has_image else ops)
...
def resolve_operations_from_intent(
    ...
    *,
    ...
    if _contains_any(text, _FACE_DETAIL_PHRASES):
        operations.append("face_detail")
    elif _contains_any(text, ("smile", "expression")) or re.search(r"\b(face|eyes|hands)\b", text):
...
def _qwen_edit_requested(settings: dict[str, Any]) -> bool:
    edit_type = str(settings.get("edit_type") or "").lower()
    cn_type = str(settings.get("cn_type") or "").lower()
    model = " ".join(
        str(settings.get(key) or "")
        for key in ("model", "base_model", "engine_name", "relative_path")
    ).lower()
    if edit_type == "qwen_edit" or cn_type == "qwen_edit":
        return True
    if "qwen" in model and "edit" in model:
        return True
    perf = str(settings.get("performance") or "").strip().lower()
    if edit_type in {"auto", ""} and perf in {"lightning", "speed", "lcm"}:
        if "qwen" in model or str(settings.get("style") or "").lower() == "image_edit":
            return True
    return False
...
def assess_workflow_readiness(
    ...
    *,
    ...
    if "flux_fill_checkpoint" in required_models:
        fams = get_families_for_capability(ModelCapabilities.FLUX_FILL_INPAINT)
        if not _has_capable_model(("checkpoints", "diffusion_models", "unet"), fams):
            missing_models.append("flux_fill_checkpoint")
...
def _downloadable_resource_entries(
    *,
    ...
    if "qwen_image_edit" in template_ids:
        try:
            from dreamforge_cli_inventory import (
                qwen_fused_lightning_model,
                qwen_lightning_lora_requested,
                qwen_lightning_lora_requirement,
                qwen_specific_lightning_lora_present,
            )
            from dreamforge_companion_download import enrich_missing_dependency

            perf = current_settings.get("performance")
            model_name = " ".join(
                str(current_settings.get(key) or "")
                for key in ("model", "base_model", "engine_name", "relative_path")
            )
            if qwen_lightning_lora_requested(perf) and not qwen_fused_lightning_model(model_name):
                req = qwen_lightning_lora_requirement(perf)
                if not qwen_specific_lightning_lora_present(req):
                    payload = {
                        **req,
                        "expected_path": str(MODELS_ROOT / req["relative"]),
                        "resource": "qwen_lightning_lora",
                        "template_ids": list(template_ids),
                    }
                    enriched = enrich_missing_dependency(payload)
                    resource_id = str(enriched.get("id") or "")
...
def missing_custom_node_pack_entries(
