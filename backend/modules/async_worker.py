...
interrupt_dreamforge_processing = False
...
def _process(gen_data):
    ...
    def callback(step, x0, x, total_steps, y):
        global status, interrupt_dreamforge_processing

        if interrupt_dreamforge_processing:
    ...
    for i in range(max(image_number, 1)):
        ...
        for x in imgs:
            ...
            #     comment = f"{handle_whitespace(p_txt)}\nNegative prompt: {handle_whitespace(n_txt)}\nSteps: {round(steps, 1)}, Sampler: {gen_data['sampler_name']} {gen_data['scheduler']}, CFG Scale: {float(gen_data['cfg'])}, Seed: {seed}, Size: {width}x{height}, Model hash: {model_hash(Path(shared.path_manager.model_paths['modelfile_path']) / gen_data['base_model_name'])}, Model: {gen_data['base_model_name']}, Version: DreamForge"
...
def worker():
    ...
    # Headless desktop/CLI workers load the user's chosen model per job.
    # Preloading the default SDXL checkpoint here wastes VRAM and can race
    # with the first real generation (access violations on Windows).
