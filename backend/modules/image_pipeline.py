...
def _worker():
    import modules.async_worker as w
    return w
...
from comfy.sd import load_state_dict_guess_config, CLIP as ComfyCLIP
from comfy.supported_models import HiDreamO1 as HiDreamO1ModelConfig
...
class pipeline:
    ...
    def load_hidream_o1_clip_vae(self):
        """HiDream-O1 uses a tokenizer-only TE and pixel-space VAE baked into Comfy, not legacy HiDream CLIPs."""
        cfg = HiDreamO1ModelConfig()
        clip_target = cfg.clip_target()
        clip_sd = cfg.process_clip_state_dict({})
        embed_dirs = path_manager.get_folder_paths("embeddings")
        ...
        vae_sd = cfg.process_vae_state_dict({})
        vae = VAE(sd=vae_sd)
        return clip, vae

    def resolve_aux_model_file(self, filename, path_key, extra_model_dirs=None):
        """Find shared encoders/VAEs in both legacy DreamForge and Comfy-style folders."""
        if filename is None:
            return None
        roots = path_manager.model_paths.get(path_key)
        roots = roots if isinstance(roots, list) else [roots]
        models_root = Path(__file__).resolve().parents[1] / "models"
        for folder in extra_model_dirs or []:
            roots.append(models_root / folder)

        for root in roots:
            candidate = Path(root) / filename
            if candidate.is_file():
                return str(candidate)
        return str(Path(roots[0]) / filename)
    ...
    def load_base_model(self, name, unet_only=False, input_unet=None, hash=None):
        ...
        if filename.endswith(".gguf") or unet_only:
            with torch.torch.inference_mode():
                try:
                    ...
                    if model_info.get("hidream_o1_native"):
                        print("Loading HiDream-O1 built-in tokenizer TE and pixel-space VAE")
                        clip, vae = self.load_hidream_o1_clip_vae()
                        clip_vision = None
                    else:
                        clip_paths = []
                        for clip_name in model_info['clip_names']:
                            clip_paths.append(
                                self.resolve_aux_model_file(
                                    clip_name,
                                    "clip_path",
                                    extra_model_dirs=["text_encoders"],
                                )
                            )
                        ...
                        vae_path = self.resolve_aux_model_file(model_info['vae_name'], "vae_path")
        ...
        if unet == None:
            ...
            if not (self.xl_base.unet.model.__class__.__name__ in self.known_models):
                print(
                    f"Model {self.xl_base.unet.model.__class__.__name__} not supported. DreamForge supports {list(self.known_models)} models as the base model."
                )
                self.xl_base = None
            ...
            if self.xl_base is not None:
                ...
                if self.model_info.get("hidream_o1_native") and (
                    self.xl_base.clip is None or self.xl_base.vae is None
                ):
                    print("HiDream-O1 checkpoint missing TE/VAE weights; using built-in tokenizer TE and pixel VAE")
                    clip_fix, vae_fix = self.load_hidream_o1_clip_vae()
                    self.xl_base = self.StableDiffusionModel(
                        unet=self.xl_base.unet,
                        clip=clip_fix if self.xl_base.clip is None else self.xl_base.clip,
                        vae=vae_fix if self.xl_base.vae is None else self.xl_base.vae,
    ...
    def process(
        ...
        try:
            if self.xl_base_patched == None or self.xl_base_patched.unet.model.__class__.__name__ not in self.known_models:
                ...
                _worker().interrupt_dreamforge_processing = True
            ...
            if "[" in positive_prompt and "]" in positive_prompt:
                if callback is not None:
                    _worker().add_result(
                        gen_data["task_id"],
                        "preview",
                        (-1, f"Processing prompt switch syntax ...", None)
                    )
