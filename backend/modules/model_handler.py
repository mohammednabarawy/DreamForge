...
class Models:
    ...
    def civit_update_worker(self, model_type, folder_paths):
        ...
        if os.environ.get("DREAMFORGE_HEADLESS") == "1":
            self._quick_list_models(model_type, folder_paths)
            return
        ...
        try:
            ...
            _log(f"Can't find imageio.v3 module: Skip CivitAI update")
            return
        if str(model_type) in self.civit_workers:
            # Already working on this folder
            _log(t("Skip CivitAI check. Update for {type} already running.", mapping={'type': model_type}))
            return
        if not Path(self.cache_paths[model_type]).is_dir():
            _log(t("WARNING: Can't find {path} Will not update thumbnails.", mapping={'path': self.cache_paths[model_type]}))
            return
        ...
        if self.offline or os.environ.get("DREAMFORGE_HEADLESS") == "1":
            if str(model_type) in self.civit_workers:
                ...
                if path.suffix.lower() in self.EXTENSIONS:
                    ...
                    if model_type == "inbox":
                        ...
                        if folder is None or baseModel is None:
                            _log(t('Skipping {name} not sure what {type} is.', mapping={'name': str(name), 'type': self.get_model_type(model_data)}))
                            updated += 1
                            continue
                        ...
                        try:
                            ...
                            _log(f"ERROR: {name} doesn't have a sha256 hash")
                            hash_sha256 = ""
                        ...
                        if cache_file.with_suffix(".json").exists():
                            ...
                            if hash_sha256 == old_data["files"][0]["hashes"]["SHA256"]:
                                if Path(dest / name).exists():
                                    _log(f"WARNING: {name} in Inbox already exists. Ignoring.")
                                else:
                                    move = True
                            else:
                                _log(f"WARNING: Renaming to {name}-{sha256} to avoid overwriting old model.")
                                rename = True
                                move = True
                        ...
                        if move:
                            ...
                            _log(t("Moved {name} to {dest}", mapping={'name': name, 'dest': dest}))
                            updated += 1
        ...
        if updated > 0:
            _log(t("CivitAI update for {type} done.", mapping={'type': model_type}))
        self.civit_workers.remove(str(model_type))
    ...
    def _quick_list_models(self, model_type, folder_paths):
        """Fast model listing for desktop headless boot (no CivitAI / preview pass)."""
        if str(model_type) in self.civit_workers:
            return
        self.civit_workers.append(str(model_type))
        self.ready[model_type] = False
        ...
        for folder in folder_paths:
            ...
            try:
                ...
                _log(f"WARNING: model scan skipped for {root}: {exc}")
        self.names[model_type] = sorted(
            names,
            key=lambda x: (
                f"0{x.casefold()}"
    ...
    def __init__(self, offline=False):
        ...
        for cache_dir in self.cache_paths.values():
    ...
    def model_sha256(self, filename):
        _log(t("Hashing {filename}", mapping={'filename': filename}))
        blksize = 1024 * 1024
        ...
        try:
            ...
            _log(f"model_sha256(): Failed reading {filename}")
            _log(f"Error: {e}")
            ret = None
        return ret
    ...
    def search_civitai_with_hash(self, hash):
        ...
        try:
            ...
            if response.status_code in [404, 451]:
                ...
                _log("Error: Civit.ai Service Currently Unavailable")
            else:
                _log(f"HTTP Error: {e}")
    ...
    def get_models_by_path(self, model_type, path):
        ...
        if data is not None:
            _log(t("Update model data: {path}", mapping={'path': json_path}))
    ...
    def get_model_path(self, model_type, name, hash=None, default=None):
        ...
        if filename is None:
            _log(f"Could not find file: {name}")
            if hash is not None and hash != "None":
                _log(f"    SHA256: {hash}")
                data = self.search_civitai_with_hash(hash)
                if data is not None and data.get('modelId', None) is not None:
                    _log(f"    Download link: https://civitai.com/models/{data.get('modelId')}")
                _log(f"    Search here: https://civitaiarchive.com/sha256/{hash}")
    ...
    def get_image(self, model, path):
        ...
        for preview in model.get("images", [{}]):
            ...
            if url:
                _log(t("Updating preview for {text}.", mapping={'text': caption_text}))
                image_url = url
                response = self.session.get(image_url)
                if response.status_code != 200:
                    _log(f"WARNING: get_image() for {caption_text} - {response.status_code} : {response.reason}")
                    break
