...
class PathManager:
    DEFAULT_PATHS = {
        ...
        "path_vae": "../models/vae/",
        "path_preview": "../temp/previews/preview.jpg",
        "path_faceswap": "../models/faceswap/",
        "path_upscalers": "../models/upscale_models",
        "path_outputs": "../temp/comfy-staging/",
...
#                paths[key] = newpaths[key]
    ...
    def get_abspath(self, path):
        p = Path(path) if Path(path).is_absolute() else Path(__file__).parent / path
        # Windows long-path prefix (\\?\) breaks mkdir when the path still contains "..".
        text = str(p)
    ...
    def download_file(self, file_key):
        ...
        if total_size > 0 and bytes_written < total_size:
            ...
            raise IOError(
                f"Download incomplete: {bytes_written} of {total_size} bytes. "
                f"File deleted. Check your network connection and try again."
            )
