...
def _lazy_gguf():
    ...
    return load_gguf_sd, DualCLIPLoaderGGUF, GGUFModelPatcher, GGMLOps
...
class DreamForgeExpansion:
    tokenizer = None
    model = None

    def __init__(self):
        self.load_model_and_tokenizer(dreamforge_expansion_path)
    ...
    def __call__(self, prompt, seed):
        ...
        set_seed(seed)
        positive_words = (
            open(os.path.join(dreamforge_expansion_path, "positive.txt"), encoding="utf-8")
            .read()
            .splitlines()
        )
...
#                model_options["load_device"] = model_options["offload_device"] = torch.device("cpu")
            ...
            _, DualCLIPLoaderGGUF, GGUFModelPatcher, _ = _lazy_gguf()
