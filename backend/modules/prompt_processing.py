    ...
    for i in range(0, steps):
        try:
            ...
            while "[" in prompt_per_step[i]:
                ...
                if "]" not in startoflastpatterncomplete:
                    break
                endoflastpattern = prompt_per_step[i].find("]", startoflastpattern)
                if endoflastpattern == -1:
                    break
                allswitchpatterns = [
                    prompt_per_step[i][startoflastpattern + 1:endoflastpattern]
                ]

                for switchpattern in allswitchpatterns:
                    switchpattern = "[" + switchpattern + "]"
                    matchfound = False
                    prompt_before_switch = prompt_per_step[i]
                    ...
                    if prompt_per_step[i] == prompt_before_switch:
                        prompt_per_step[i] = prompt_per_step[i].replace(
                            switchpattern,
                            switchpattern.strip("[]"),
                            1,
                        )
