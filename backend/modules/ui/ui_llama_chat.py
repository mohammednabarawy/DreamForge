...
def create_chat():
    ...
    def _builtin_assistant_info():
        return {
            "name": "DreamForge",
            "greeting": "Hi! Add character folders under chatbots/ to use the LLM chat.",
            "avatar": "html/logo.png",
            "system": "You are a helpful assistant for DreamForge.",
            "embed": json.dumps([]),
            "chatstart": [{"role": "assistant", "content": "Hi! Add character folders under chatbots/ to use the LLM chat."}],
        }

    def _default_assistant_path():
        default = Path("chatbots/rf_support_troll")
        if default.is_dir() and (default / "info.json").is_file():
            return str(default)
        assistants = llama_get_assistants()
        return assistants[0][1] if assistants else None
    ...
    def _llama_select_assistant(dropdown):
        if not dropdown:
            return _builtin_assistant_info()
    ...
    with gr.Blocks() as app_llama_chat:
        with gr.Row():
            with gr.Column(scale=3), gr.Group():
                default_bot = _default_assistant_path()
                default_info = _llama_select_assistant(default_bot)
            ...
            with gr.Column(scale=2), gr.Group():
                llama_avatar = gr.Image(
                    value=default_info["avatar"],
                    label=default_info["name"],
                    height=400,
                    width=400,
                    show_label=True,
                )
                ...
                llama_system = gr.Textbox(
                    visible='hidden',
                    value=default_info["system"],
                )
                llama_embed = gr.Textbox(
                    visible='hidden',
                    value=default_info["embed"],
                )
