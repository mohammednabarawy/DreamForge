...
## Local AI Agent Wrapper

This install includes a project-level agent wrapper in `D:\DreamForge_win64_2-5-0`:

- `dreamforge_cli_direct.py` / `dreamforge-cli.bat` for headless JSON image generation.
- `dreamforge_mcp_server.py` for MCP clients.
- `arabic_poster_pipeline.py` / `arabic-poster-cli.bat` for exact Arabic/text-safe poster workflows.
- `DREAMFORGE_AGENT_SKILL.md` for agent usage rules.
- `EVALUATION_REPORT.md` for the latest local model/runtime test results.

The wrapper inventories classic checkpoints plus `diffusion_models`, `unet`, `text_encoders`, and `.gguf` models so agents can route Flux, HiDream, Qwen-Image/Edit, SD3, Flux2, Z-Image, Wan, Hunyuan, and SDXL jobs with explicit `--vram-profile 16gb` or `--vram-profile 5gb` settings.

<img src="https://raw.githubusercontent.com/runew0lf/pmmconfigs/main/DreamForge_ss.png" width=100%>

# The Future of AI Art is Here - Introducing DreamForge

Forget everything you thought you knew about AI art generation - **DreamForge** is here to completely reinvent the game! 

This groundbreaking new image creator combines the best aspects of **Stable Diffusion** and **Midjourney** into one seamless, cutting-edge experience. The days of messy installations and manual tweaking are over. With **DreamForge**, stunning images spring to life with just a few words - no technical skills required.

Built from the DNA of its predecessors but enhanced with next-level AI, **DreamForge** generates jaw-dropping visuals in seconds. Just type a prompt and watch in awe as your wildest creative visions are instantly brought to life. The possibilities are truly endless.

But the magic doesn't stop there. **DreamForge** puts the power in your hands like never before. You're in complete control of the experience from start to finish. And with its streamlined interface and intuitive controls, anyone can create like a pro.

Don't waste another minute stuck in the past. The future is calling, and it's time to answer with **DreamForge** - the new standard in AI artistry. Install it today and unlock your creative potential like never before.  
...
You can directly download DreamForge with:

**[>>> Click here to download <<<](https://github.com/runew0lf/DreamForge/releases/download/Release-2.0.0/DreamForge_main_2.0.0.win64.7z)**

## NOTE YOU WILL NEED TO EXTRACT USING THE LATEST [7ZIP](https://www.7-zip.org/download.html)

After you download the file, please uncompress it, and then run the "run.bat".

![image](https://github.com/lllyasviel/DreamForge/assets/19834515/c49269c4-c274-4893-b368-047c401cc58c)

In the first time you launch the software, it will automatically download models:

1. It will download [sd_xl_base_1.0_0.9vae.safetensors from here](https://huggingface.co/stabilityai/stable-diffusion-xl-base-1.0/resolve/main/sd_xl_base_1.0_0.9vae.safetensors) as the file "DreamForge\models\checkpoints\sd_xl_base_1.0_0.9vae.safetensors".
...
The command lines are
    ...
    git clone https://github.com/runew0lf/DreamForge.git
    cd DreamForge
    virtualenv venv
    source venv/bin/activate
    pip install -r requirements_versions.txt

Then download the models: download [sd_xl_base_1.0_0.9vae.safetensors from here](https://huggingface.co/stabilityai/stable-diffusion-xl-base-1.0/resolve/main/sd_xl_base_1.0_0.9vae.safetensors) as the file "DreamForge\models\checkpoints\sd_xl_base_1.0_0.9vae.safetensors", and download [sd_xl_refiner_1.0_0.9vae.safetensors from here](https://huggingface.co/stabilityai/stable-diffusion-xl-refiner-1.0/resolve/main/sd_xl_refiner_1.0_0.9vae.safetensors) as the file "DreamForge\models\checkpoints\sd_xl_refiner_1.0_0.9vae.safetensors". **Or let DreamForge automatically download the models** using the launcher:
...
18. Drag and drop previous **DreamForge** images to the main window, it will autofill the prompt with the generation metadata.
...
43. Use [merge-files](https://github.com/runew0lf/DreamForge/wiki/Checkpoint-merges) as checkpoints.
...
This codebase is a fork from the original amazing work by [lllyasviel](https://github.com/lllyasviel/DreamForge/discussions/117)
