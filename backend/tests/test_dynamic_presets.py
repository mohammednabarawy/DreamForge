...
def test_apply_dynamic_preset_merges_recipe_without_overriding_explicit_model():
    ...
    assert settings.get("performance") == "Lightning"
...
def test_style_none_skips_recipe_prompt_fragments():
    from types import SimpleNamespace

    from dreamforge_agent_tools import apply_recipe_defaults, compile_creative_prompt

    prompt = "Premium product hero shot, studio lighting, clean negative space"
    args = SimpleNamespace(style="none", styles=[], performance="Speed")
    apply_recipe_defaults(args)
    assert getattr(args, "styles", None) in (None, [])
    compiled = compile_creative_prompt(prompt, args)
    assert compiled == prompt
    assert "advertising" not in compiled.lower()
    assert "premium product advertising campaign" not in compiled.lower()


def test_default_prompt_enhancer_is_opt_in():
    from dreamforge_prompt.pipeline import default_prompt_enhancer

    assert default_prompt_enhancer("sdxl", "generate") == "none"
    assert default_prompt_enhancer("flux", "generate") == "none"
