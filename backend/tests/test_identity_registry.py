...
def test_identity_role_override(tmp_path, monkeypatch):
    _isolated_registry(tmp_path, monkeypatch)
    identities.upsert_identity(
        {
            "name": "Portrait Lead",
            "type": "person",
            "image_paths": ["D:/refs/face.png"],
        }
    )

    settings = identities.apply_identity_to_settings(
        {"identity_id": "portrait-lead", "identity_role": "brand"}
    )

    assert settings["identity_reference"]["type"] == "person"
    assert settings["identity_reference"]["role"] == "brand"
...
def test_brain_plan_names_attached_identity(tmp_path, monkeypatch):
    ...
    assert "as person" in decision["message"]
