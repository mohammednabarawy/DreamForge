...
def _model(name, size_mb, family):
    return {
        ...
        "relative_path": name,
        "path": f"D:/DreamForge/models/checkpoints/{name}",
        "size_mb": size_mb,
        "family": family,
    }
...
def test_router_prefers_q4_gguf_for_lightning_edit_route(monkeypatch):
    import dreamforge_cli_inventory as inv

    q4 = _model("qwen-image-edit-2511-Q4_K_M.gguf", 12_632, "qwen_image_edit")
    fp8_lightning = _model(
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "categories": {
                "checkpoints": [],
                "diffusion_models": [fp8_lightning, q4],
                "unet": [],
            }
        },
    )
    monkeypatch.setattr(inv, "check_model_dependencies", lambda _model, **kwargs: [])

    chosen = inv.route_best_model({ModelCapabilities.IMAGE_TO_IMAGE}, "16gb", "Lightning")

    assert chosen["name"] == "qwen-image-edit-2511-Q4_K_M.gguf"


def test_router_prefers_qwen_2511_q4_for_ready_edit_route(monkeypatch):
    import dreamforge_cli_inventory as inv

    qwen = _model("qwen-image-edit-2511-Q4_K_M.gguf", 12_632, "qwen_image_edit")
    kontext = _model("flux1-dev-kontext_fp8_scaled.safetensors", 11_353, "flux_kontext")
    monkeypatch.setattr(
        inv,
        "list_model_inventory",
        lambda: {
