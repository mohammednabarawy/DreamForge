...
def test_nvfp4_model_blocks_when_torch_lacks_fp4_dtype(tmp_path, monkeypatch):
    models_dir = tmp_path / "models" / "checkpoints"
    _write_fake_safetensors(models_dir / "qwenImage2512Nvfp4_v10.safetensors")
    monkeypatch.setattr(pf, "MODELS_ROOT", tmp_path / "models")
    monkeypatch.setattr(pf, "DEFAULT_OUTPUT_ROOT", tmp_path / "outputs")
    monkeypatch.setattr(pf, "_disk_free_gb", lambda _p: 50.0)
    monkeypatch.setattr(pf, "_torch_supports_nvfp4", lambda: False)
    model = {
    ...
    assert result.has_errors
    assert result.errors[0]["code"] == "unsupported_model_format"
    assert result.errors[0]["details"]["required_torch_attribute"] == "float4_e2m1fn_x2"
