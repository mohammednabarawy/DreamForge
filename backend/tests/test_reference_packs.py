...
def test_reference_pack_role_override(isolated_pack_store: Path):
    packs.upsert_reference_pack(
        {
            "name": "Noir Character",
            "type": "character",
    ...
    settings = packs.apply_reference_pack_to_settings(
        {"reference_pack_id": "noir-character", "reference_pack_role": "brand"}
    )

    assert settings["reference_pack"]["type"] == "character"
    assert settings["reference_pack"]["role"] == "brand"
...
def test_brain_plan_names_attached_reference_pack(isolated_pack_store: Path):
    ...
    assert "as brand" in decision["message"]
