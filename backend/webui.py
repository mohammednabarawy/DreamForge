...
from modules.model_ui_defaults import (
    format_model_current_html,
    gallery_model_type_label,
    list_gallery_models,
...
def launch_app(args):
    ...
    favicon_path = str(Path(__file__).resolve().parent / "html" / "logo.png")
    ...
    main_tabs = gr.TabbedInterface(
        ...
        title="DreamForge " + version.version,
...
def generate_clicked(*args):
    ...
    if gen_data['preset_selection']:
        try:
            ...
            if preset_data["software"] == "DreamForge":
...
with shared.gradio_root as block:
    ...
    gr.HTML(
        value="""
        <div class="dreamforge-brand-bar">
          <img src="gradio_api/file=html/logo-wordmark.png" alt="DreamForge" class="dreamforge-brand-wordmark"/>
        </div>
        """,
        elem_id="dreamforge-brand-header",
    )
        ...
        with gr.Column(scale=2) as right_col:
            ...
            with model_tab:
                with gr.Tab(label=t("Model")):
                    _startup_civit = gallery_model_type_label(
                        "checkpoints",
                        settings["base_model"],
                        shared.models,
                    )
                    _startup_profile = resolve_ui_profile(
                        settings["base_model"],
                        current_performance=settings["performance"],
                        lock_enabled=True,
                        preset_active=False,
                    )
                    model_current = gr.HTML(
                        value=format_model_current_html(
                            settings["base_model"],
                            _startup_civit,
                            _startup_profile,
                            translate_fn=t,
                    ...
                    use_family_defaults = gr.Checkbox(
                        label=t("Use recommended settings for selected model family"),
                        value=True,
                    )
                    with gr.Group():
                    ...
                    def update_model_filter(filtered):
                        return gr.update(value=list_gallery_models(filtered))

                    model_select_outputs = [
                        model_current,
                        base_model,
                    ...
                    def _complete_model_select_updates(partial):
                        """block.load requires a value per output; fill gaps with no-op updates."""
                        return tuple(
                            partial.get(component, gr.update())
                            for component in model_select_outputs
                        )

                    def _apply_profile_gradio_updates(profile, html, engine_name):
                        partial = {
                            model_current: gr.update(value=html),
                            base_model: gr.update(value=engine_name),
                        }
                        if not profile["apply_performance"]:
                            return partial
                        perf = profile["performance_selection"]
                        partial[performance_selection] = gr.update(value=perf)
                        custom = profile.get("custom_sampling")
                        if perf == performance_settings.CUSTOM_PERFORMANCE and custom:
                            partial[custom_steps] = gr.update(value=custom["custom_steps"])
                            partial[cfg] = gr.update(value=custom["cfg"])
                            partial[sampler_name] = gr.update(value=custom["sampler_name"])
                            partial[scheduler] = gr.update(value=custom["scheduler"])
                            partial[clip_skip] = gr.update(value=custom["clip_skip"])
                        elif perf != performance_settings.CUSTOM_PERFORMANCE:
                            opts = performance_settings.get_perf_options(perf)
                            partial[custom_steps] = gr.update(value=opts["custom_steps"])
                            partial[cfg] = gr.update(value=opts["cfg"])
                            partial[sampler_name] = gr.update(value=opts["sampler_name"])
                            partial[scheduler] = gr.update(value=opts["scheduler"])
                            partial[clip_skip] = gr.update(value=opts["clip_skip"])
                        if profile["clear_styles"]:
                            partial[style_selection] = gr.update(value=[])
                        if profile["clear_negative"]:
                            partial[negative_prompt] = gr.update(value="")
                        return partial
                    ...
                    def update_model_select(
                        ...
                        caption = f"{evt.value['caption']}"
                        category, relative_name = parse_gallery_caption(caption)
                        civit_base = gallery_model_type_label(
                            category,
                            relative_name,
                            shared.models,
                        )
                        preset_active = bool(preset_path)

                        profile = resolve_ui_profile(
                            relative_name,
                            category=category,
                            current_performance=current_performance,
                            lock_enabled=lock_defaults,
                            preset_active=preset_active,
                        )
                        html = format_model_current_html(
                            caption,
                            civit_base,
                            profile,
                            translate_fn=t,
                        )
                        return _apply_profile_gradio_updates(
                            profile,
                            html,
                            profile["engine_name"],
                        )
                    ...
                    model_gallery.select(
                        ...
                        outputs=model_select_outputs,
                    )

                    def apply_startup_model_defaults(
                        current_performance,
                        lock_defaults,
                        preset_path,
                    ):
                        if preset_path:
                            return _complete_model_select_updates({})
                        profile = resolve_ui_profile(
                            settings["base_model"],
                        ...
                        partial = _apply_profile_gradio_updates(
                            profile,
                            html,
                            profile["engine_name"],
                        )
                        return _complete_model_select_updates(partial)

                    block.load(
                        fn=apply_startup_model_defaults,
                        inputs=[
                            performance_selection,
                            use_family_defaults,
                            preset_selection,
            ...
            def model_refresh_clicked(lora_active_gallery):
                ...
                results = {
                    ...
                    model_gallery: gr.update(value=list_gallery_models("")),
                ...
                with gr.Row():
                    gr.HTML(
                        ...
                        <div style="text-align:center;margin:8px 0 14px;">
                          <img src="gradio_api/file=html/logo-icon.png" alt="" width="64" height="64" style="border-radius:12px;margin-bottom:8px;"/>
                          <img src="gradio_api/file=html/logo-wordmark.png" alt="DreamForge" style="max-height:48px;width:auto;max-width:100%;"/>
                        </div>
                        ...
                        <a href="https://github.com/runew0lf/DreamForge"><img src="gradio_api/file=html/github-mark-white.svg" height="16" width="16" style="display:inline-block;">&nbsp;Github</a><br>
