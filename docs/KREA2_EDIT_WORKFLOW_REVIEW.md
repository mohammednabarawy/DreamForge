...
The Edit panel names the route and explains the input order: source/scene first, optional subject second. Two separate images feed both the patch and the grounded encoder; they are not collaged. More than two inputs produce a clear error. The separate Keep Face, character assignment, face selection, and likeness verification/retry controls have been removed from Generate/Edit. Old identity flags and reference face metadata are ignored for normal Generate/Edit requests, so they cannot override the selected editor. The required Krea Identity Edit v1.2 LoRA remains part of the native workflow. Toolbox/custom workflows and Photo Restore's separate detail pass are preserved. Generate/restyle still uses the existing Krea img2img path.
